...
<%
 /**
  * @Class Name : ShopRegister.jsp
  * @Description : Shop Register 화면
...
<c:set var="registerFlag" value="${empty shopVO.id ? '등록' : '수정'}"/>			
...
function fn_egov_selectList() {
   	document.getElementById("detailForm").action = "<c:url value='/shop/ShopList.do'/>";
   	document.getElementById("detailForm").submit();		
}

/* 글 삭제 function */
function fn_egov_delete() {
   	document.getElementById("detailForm").action = "<c:url value='/shop/deleteShop.do'/>";
   	document.getElementById("detailForm").submit();		
}
...
function fn_egov_save() {	
	...
	/* TODO Validation기능 보완 */
  	...
  	frm.action = "<c:url value="${registerFlag == '등록' ? '/shop/addShop.do' : '/shop/updateShop.do'}"/>";
    frm.submit();
...
<form:form commandName="shopVO" name="detailForm" id="detailForm" >
<div id="content_pop">
	<!-- 타이틀 -->
	<div id="title">
		<ul>
	...
	<table width="100%" border="1" cellpadding="0" cellspacing="0" >
		...
		<tr>
			<th>itemdesc</th>
			<td>
				<form:input path="itemdesc" cssClass="txt"/>
		...
		<tr>
			<th>cost</th>
			<td>
				<form:input path="cost" cssClass="txt"/>
