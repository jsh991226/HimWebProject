...
<%
 /**
  * @Class Name : ReplyRegister.jsp
  * @Description : Reply Register 화면
...
<c:set var="registerFlag" value="${empty replyVO.id ? '등록' : '수정'}"/>
...
function fn_egov_selectList() {
   	document.getElementById("detailForm").action = "<c:url value='/reply/ReplyList.do'/>";
   	document.getElementById("detailForm").submit();		
}

/* 글 삭제 function */
function fn_egov_delete() {
   	document.getElementById("detailForm").action = "<c:url value='/reply/deleteReply.do'/>";
   	document.getElementById("detailForm").submit();		
}
...
function fn_egov_save() {	
	...
	/* TODO Validation기능 보완 */
  	...
  	frm.action = "<c:url value="${registerFlag == '등록' ? '/reply/addReply.do' : '/reply/updateReply.do'}"/>";
    frm.submit();
...
<form:form commandName="replyVO" name="detailForm" id="detailForm" >
<div id="content_pop">
	<!-- 타이틀 -->
	<div id="title">
		<ul>
	...
	<table width="100%" border="1" cellpadding="0" cellspacing="0" >
	   ...
	   <tr>
		...
		<tr>
			<th>owner</th>
			<td>
				<form:input path="owner" cssClass="txt"/>
		...
		<tr>
			<th>replygroup</th>
			<td>
				<form:input path="replygroup" cssClass="txt"/>
				&nbsp;<form:errors path="replygroup" />
			</td>
		</tr>	
	</table>
  </div>
